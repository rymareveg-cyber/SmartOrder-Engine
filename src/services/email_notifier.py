...
def send_tracking_email(
    ...
    try:
        ...
        if is_shipped:
            ...
            subject    = f"💳 Оплата подтверждена · Заказ #{order_number} — трек-номер присвоен"
            header_txt = "💳 Оплата подтверждена · Трек-номер присвоен!"
            header_bg  = "linear-gradient(135deg,#388e3c,#1b5e20)"
            intro_line = (
                f"Оплата по заказу <strong>#{order_number}</strong> подтверждена. "
                f"Трек-номер присвоен — посылка формируется на складе. "
                f"Как только её передадут курьеру, вы получите ещё одно письмо."
            )
        ...
        if is_shipped:
            ...
            plain_intro = (
                f"Оплата по заказу #{order_number} подтверждена. Трек-номер присвоен.\n"
