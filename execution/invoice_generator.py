...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/invoice_generator.log'),
...
# Директория для сохранения счётов
...
def register_cyrillic_fonts():
    ...
    """
    try:
        # Попытка использовать системные шрифты Windows
        import platform
        if platform.system() == 'Windows':
            ...
            if registered_font:
                # Если нашли обычный шрифт, но не нашли жирный, используем тот же
                if not registered_bold:
                    registered_bold = registered_font
                return registered_font, registered_bold
        
        # Fallback: использование Unicode CID шрифтов (встроенные в reportlab)
        try:
        ...
        # Если ничего не получилось, используем стандартные шрифты (с предупреждением)
...
def generate_invoice_number(order_date: datetime) -> str:
    ...
    # Простая генерация: используем timestamp для уникальности
    # В реальной системе лучше использовать счётчик в БД
...
def generate_invoice_pdf(order: Order, invoice_number: str) -> str:
    ...
    try:
        ...
        for idx, item in enumerate(order.items, 1):
            ...
            # Обрезка длинных названий для таблицы
            product_name = item.product_name
            if len(product_name) > 35:
                product_name = product_name[:32] + '...'
            
            # Форматирование цен без символа рубля (для таблицы)
        ...
        # Создание таблицы (улучшенные размеры колонок)
        ...
        items_table.setStyle(TableStyle([
            ...
            ('ALIGN', (1, 1), (1, -1), 'LEFT'),  # Артикул
            ('ALIGN', (2, 1), (2, -1), 'LEFT'),  # Наименование
            ('ALIGN', (3, 1), (3, -1), 'CENTER'),  # Кол-во
            ('ALIGN', (4, 1), (4, -1), 'RIGHT'),  # Цена
            ('ALIGN', (5, 1), (5, -1), 'RIGHT'),  # Сумма
...
class InvoiceGenerator:
    ...
    def generate_invoice(order_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Проверка статуса (должен быть validated)
            ...
            # Обновление статуса заказа на invoice_created
            try:
...
if __name__ == "__main__":
    ...
    # Для тестирования нужен реальный order_id из БД
    print("Invoice Generator ready")
    print("Usage: InvoiceGenerator.generate_invoice(order_id)")
