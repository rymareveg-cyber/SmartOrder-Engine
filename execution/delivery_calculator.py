...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('logs/delivery_calculator.log'),
...
def calculate_weight_from_items(items: list) -> float:
    ...
    Использует приблизительный вес: 1 кг на товар (можно улучшить, добавив вес в каталог).
    ...
    # Приблизительный вес: 1 кг на товар
    # В будущем можно добавить поле weight в каталог товаров
...
class DeliveryCalculator:
    ...
    def calculate(
        ...
        try:
            ...
            # Если вес превышает все диапазоны, используем последний
            if cost is None:
            ...
            # Fallback на максимальную стоимость
            return {
                "city": city or "unknown",
                "weight": weight or 1.0,
                "cost": 2000,  # Максимальная стоимость
                "estimated_days": 7,
                "carrier": "local"
            }
    ...
    def calculate_for_order(self, order_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if address:
            # Простая эвристика: ищем название города в начале адреса
...
if __name__ == "__main__":
    ...
    # Тест 1: Москва, легкий вес
    result1 = calculator.calculate("Москва", weight=3.0)
    print(f"Test 1: {result1}")
    assert result1["cost"] == 500
    
    # Тест 2: Санкт-Петербург, средний вес
    result2 = calculator.calculate("Санкт-Петербург", weight=7.0)
    print(f"Test 2: {result2}")
    assert result2["cost"] == 900
    
    # Тест 3: Другой город, тяжелый вес
    result3 = calculator.calculate("Казань", weight=12.0)
    print(f"Test 3: {result3}")
    assert result3["cost"] == 2000
    
    # Тест 4: Расчёт из товаров
    items = [
        {"quantity": 2, "product_name": "Товар 1"},
        {"quantity": 1, "product_name": "Товар 2"}
    ]
    result4 = calculator.calculate("Москва", items=items)
