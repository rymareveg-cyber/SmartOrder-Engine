...
**Задачи**:
- Реализация экспорта счетов в 1С (при статусе "paid", после оплаты)
- Реализация retry логики
- Реализация dead letter queue
- Логирование всех экспортов
- Интеграция с payment_processor для автоматического экспорта после оплаты

**Результат**: Синхронизация счетов с 1С после оплаты
