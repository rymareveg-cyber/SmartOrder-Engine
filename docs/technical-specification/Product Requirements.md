...
**SmartOrder Engine** - AI-система автоматической обработки заказов в e-commerce с интеграцией 1С:Управление небольшой фирмой

### 1.2. Версия документа
**v1.0** от 13.02.2026

### 1.3. Описание проекта
Создание интеллектуальной системы для автоматического приёма, обработки и управления заказами из различных каналов связи с использованием AI для понимания запросов на естественном языке и автоматической интеграцией с 1С:Управление небольшой фирмой.
...
- **FR-1.1**: Система должна синхронизировать каталог товаров из 1С:Управление небольшой фирмой через HTTP Service
...
- **FR-6.1**: Система должна экспортировать счёт в 1С при статусе "invoice_created" (ДО оплаты клиентом)
...
#### 3.6.2. Экспорт заказов
- **FR-6.5**: Система должна экспортировать заказ в 1С при статусе "paid" (ПОСЛЕ оплаты клиентом)
- **FR-6.6**: Формат данных должен соответствовать API 1С
- **FR-6.7**: Система должна обрабатывать ответ 1С
- **FR-6.8**: Система должна помечать заказ как экспортированный (флаг order_exported_to_1c)

#### 3.6.3. Обработка ошибок
...
- **NFR-20**: Совместимость с 1С:Управление небольшой фирмой 8.3+
...
#### Внешние API
- **1С HTTP Service**: синхронизация каталога, экспорт заказов/счетов
...
- **1С HTTP Service** - веб-сервис 1С:Управление небольшой фирмой для интеграции
